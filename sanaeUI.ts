...
export async function delayPrint(element:HTMLElement,text:string,interval:number,add:boolean = false):Promise<any>{
    // 指定された element が書き込み中でないかを確かめるラムダ式
    const is_existing = ():boolean => { return writingElements.includes(element); };
    // element の文字数を取得するラムダ式
    const get_length = ():number => { return element?.textContent?.length ?? 0; };
